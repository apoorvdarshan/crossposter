...
export function xPythonCommand(profileId?: string): string {
...
/**
 * Runs an X Playwright helper script through the configured Python and returns
 * its parsed JSON result. The script path must be scriptArgs[0]. The execFile
 * timeout is given a buffer beyond the in-script wait so the script can emit its
 * own friendly timeout message before the process is force-killed.
 */
...
/**
 * Opens a visible browser window for a one-time X login and persists the session
 * into this profile's isolated browser data directory. Headless publishing
