...
Opens a real browser window so the user can sign in (including 2FA / checkpoints)
once. The authenticated session is persisted into the profile's user-data
directory and reused headlessly by x_browser_publish.py afterward. The profile
is isolated and never touches the user's personal Chrome.
"""
import argparse
import time
...
def run_login(context, timeout_ms):
    ...
    return {
        "ok": False,
        "message": (
            "X login timed out. Re-run Log in to X and finish signing in "
